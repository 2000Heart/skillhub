...
public class AuthMethodCatalog {
    ...
    public List<AuthMethodResponse> listMethods(String returnTo) {
        ...
        if (dingTalkProperties.isEnabled()) {
            methods.add(new AuthMethodResponse(
                ...
                "dingtalk",
                "DingTalk",
                buildDingTalkAuthorizeUrl(sanitizedReturnTo)
            ));
        }
    ...
    private String buildDingTalkAuthorizeUrl(String returnTo) {
        String baseUrl = "/api/v1/auth/dingtalk/oauth/authorize";
        if (returnTo == null) {
            return baseUrl;
        }
        return baseUrl + "?returnTo=" + URLEncoder.encode(returnTo, StandardCharsets.UTF_8);
