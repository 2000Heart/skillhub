...
import { DingTalkLoginPanel } from '@/features/auth/dingtalk/dingtalk-login-panel'
import { getDirectAuthRuntimeConfig, getDingTalkRuntimeConfig } from '@/api/client'
...
export function LoginPage() {
  ...
  const showDingTalkMode = dingTalkRuntime.enabled
  ...
  const oauthFailed = search.reason === 'dingtalkOAuthFailed'

  const { data: authMethods } = useAuthMethods(showDingTalkMode ? undefined : search.returnTo)
        ...
        <div className="w-full max-w-md space-y-8 animate-fade-up">
          <div className="text-center space-y-3">
            ...
            <p className="text-muted-foreground text-lg">
              {t('login.dingtalkSubtitle', { defaultValue: '使用钉钉账号登录 SkillHub' })}
            </p>
          </div>
